...
export default function AdminEssaysPressList() {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            const { data, error } = await supabase
                .from("essays_press")
                .select("id, year, title, sort_order, images, image, files, file_url, file_name")
    ...
    const handleDelete = async (id) => {
        ...
        const { error } = await supabase.from("essays_press").delete().eq("id", id);
        if (error) {
    ...
    const handleSaveOrder = async () => {
        ...
        try {
            const results = await Promise.all(
                items.map((item, idx) =>
                    supabase.from("essays_press").update({ sort_order: idx + 1 }).eq("id", item.id).select("id")
                )
            );
