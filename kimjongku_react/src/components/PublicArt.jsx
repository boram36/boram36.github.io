...
// 샘플 데이터
const initialItems = [
  { id: 1, year: 2025, text: '퍼블릭 아트 설명', image: null },
  { id: 2, year: 2024, text: '퍼블릭 아트 내용', image: '/assets/sample5.jpg' },
];

function PublicArt({ isAdmin = false }) {
  const [items, setItems] = useState(initialItems);
  const [newItem, setNewItem] = useState({ year: new Date().getFullYear(), text: '', image: null });
  ...
  const handleAdd = () => {
    ...
    setNewItem({ year: new Date().getFullYear(), text: '', image: null });
  };
          ...
          <li key={item.id} style={{ marginBottom: '1rem', borderBottom: '1px solid #eee', paddingBottom: '1rem' }}>
            {editId === item.id ? (
              ...
              <div>
                <span>{item.year}</span>
                <div>{item.text}</div>
                {item.image && <img src={item.image} alt="public-art-img" style={{ width: 120, marginTop: 8, border: '1px solid #ccc' }} />}
                {isAdmin && (
                  <>
