...
// 샘플 데이터
const initialItems = [
  { id: 1, year: 2025, text: '에세이 및 기사 내용', image: null, file: null, fileName: '' },
  { id: 2, year: 2024, text: '기사 내용', image: '/assets/sample3.jpg', file: null, fileName: '' },
];

function EssaysPress({ isAdmin = false }) {
          ...
          <li key={item.id} style={{ marginBottom: '1rem', borderBottom: '1px solid #eee', paddingBottom: '1rem' }}>
            {editId === item.id ? (
              ...
              <div>
                <span>{item.year}</span>
                <div>{item.text}</div>
                {item.image && <img src={item.image} alt="essay-img" style={{ width: 120, marginTop: 8, border: '1px solid #ccc' }} />}
                {item.file && (
